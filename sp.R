...
total_claimed_amount1<-
  (sum(number_of_accidents))*mean(cost_of_a_claim1) 
#expected total of the claimed amount
annual_profit_A1=total_premium_paid1 - total_claimed_amount1 
#expected annual profit once stability has been reached for Country A under discount scheme 1

######Calculation for the expected annual profit from country B once stability has been reached under the discount scheme with 3 discount levels
premium2<-300
discount_levels<-c(0, 0.15, 0.3)
...
#calculating total claims now
number_of_accidents<- rpois(number_of_customers,lambda) # number of accidents per customer
cost_of_a_claim2 <- runif(number_of_customers,0,600)
mean(cost_of_a_claim2) # expected amount of a claim
total_claimed_amount2<-
  (sum(number_of_accidents))*mean(cost_of_a_claim2) #expected total of the claimed amount
annual_profit_B1=total_premium_paid2 - total_claimed_amount2 # expected annual profit once stability has been reached for Country B under discount scheme 1
...
#######Calculation for the expected annual profit from country A once stability has been reached under the discount scheme with 4 discount levels
lambda<- 0.873 # from 0.01*(87+(3*0.1))
number_of_customers<-10000 #number of customers
n<-30 # number of steps to simulate
...
mean(cost_of_a_claim1) # expected amount of a claim
total_claimed_amount1<-
sum(number_of_accidents)*mean(cost_of_a_claim1) #expected total of the claimed amount
annual_profit_A2=total_premium_paid1 - total_claimed_amount1 # expected annual profit once stability has been reached for Country A under 2nd discount scheme

######Calculation for the expected annual profit from country B once stability has been reached under the discount scheme with 4 discount levels
...
mean(cost_of_a_claim2) # expected amount of a claim
total_claimed_amount2<-sum(number_of_accidents)*mean(cost_of_a_claim2) #expected total of the claimed amount
annual_profit_B2=total_premium_paid2 - total_claimed_amount2 # expected annual profit once stability has been reached for Country B under 2nd discount scheme
#plotting the progression of the proportion at each state
plot(levels[1,], type='l', xlim=c(0,n+1), ylim=c(0,1), xlab='Number of years',
     ylab='Proportion', col='orange',lwd=2)
lines(levels[2,], col='green', lwd=2)
